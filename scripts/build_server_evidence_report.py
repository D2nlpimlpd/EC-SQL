...
def category(row: dict[str, str]) -> str:
    ...
    if suite == "spider2-sqlite" and system == "ecsql":
        return "sqlite_ecsql_full"
    ...
    if suite == "spider2-dbt" and "ecsql_deterministic_full" in system:
        return "dbt_ecsql_full"
...
def latex_snippet(check: Check, rows: list[dict[str, str]]) -> str:
    best_sqlite = best_row(rows, categories={"sqlite_ecsql_full"})
    best_baseline = best_row(rows, categories={"sqlite_sota_baseline"})
    best_dbt = best_row(rows, categories={"dbt_ecsql_full"})
    ...
    lines = [
        ...
        latex_escape(metric_sentence(best_sqlite, "Best EC-SQL SQLite result")),
...
def markdown_report(run_id: str, check: Check, rows: list[dict[str, str]]) -> str:
    ...
    lines = [
        ...
        "## Key Results",
        "",
        f"- {metric_sentence(best_sqlite, 'Best EC-SQL SQLite result')}",
